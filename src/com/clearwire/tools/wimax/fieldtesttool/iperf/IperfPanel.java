...
public class IperfPanel extends JPanel implements ActionListener, ItemListener{
  ...
  private JRadioButton numbuff, transTime;
  ...
  private JLabel sshServerLabel = new JLabel("Remote SSH Server");
  private JTextField sshServerField = new JTextField();
  private JLabel sshUsernameLabel = new JLabel("SSH Username");
  private JTextField sshUsernameField = new JTextField();
  private JLabel sshPasswordLabel = new JLabel("SSH Password");
  private JPasswordField sshPasswordField = new JPasswordField();
  private boolean remoteIperf=false;
  ...
  String version;
  	...
  	//remote iperf contructor
  public IperfPanel(String v,String serverAddress,String portNum,String sshAddress,String sshUsername,String sshPassword){
	  this(v,serverAddress,portNum);
	  	remoteIperf=true;
	  	Box sshBox = new Box(BoxLayout.Y_AXIS);
		JPanel sshServerPanel = new JPanel();
		sshServerPanel.add(sshServerLabel);
		sshServerPanel.add(sshServerField);
		sshServerField.setColumns(10);
		sshServerField.setText(sshAddress);
		sshBox.add(sshServerPanel);
		
		JPanel sshUsernamePanel = new JPanel();
		sshUsernamePanel.add(sshUsernameLabel);
		sshUsernamePanel.add(sshUsernameField);
		sshUsernameField.setColumns(10);
		sshUsernameField.setText(sshUsername);
		sshBox.add(sshUsernamePanel);
		
		JPanel sshPasswordPanel = new JPanel();
		sshPasswordPanel.add(sshPasswordLabel);
		sshPasswordPanel.add(sshPasswordField);
		sshPasswordField.setColumns(10);
		sshPasswordField.setText(sshPassword);
		sshBox.add(sshPasswordPanel);
		
		connectionInfo.add(sshBox);
  }

  	//local iperf constructor
  public IperfPanel(String v,String serverAddress,String portNum){    
	  super();
    ...
    //set up main panels
    createConnectionPanel(serverAddress,portNum);
    ...
    mainPanel.add(connectionInfo,BorderLayout.NORTH);
    mainPanel.add(tabbedPane,BorderLayout.CENTER);
    icon = new ImageIcon("images/middle.gif");
    tabbedPane.addTab("Settings", icon, settings, "Iperf Settings");
    ...
    this.add(mainPanel);   
	  ...
	  /**
	 * @return the sshAddress
	 */
	public String getSshAddress() {
		return sshServerField.getText().trim();
	}
	...
	public String getSshUsername() {
		return sshUsernameField.getText().trim();
	}
	...
	public String getSshPassword() {
		return sshPasswordField.getText().trim();
	}
...
//This creates the top connection panel that lets the user select client/server mode
  ...
  public void createConnectionPanel(String serverAddress,String portNum) {
  ...
  //new panel for settings
    ...
    //general.setPreferredSize(new Dimension(600,150));
  ...
  public void createOutputPanel() {
    ...
    //outPanel.setPreferredSize(new Dimension(600,200));
    outPanel.setDividerLocation(275);
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    //run
      ...
      if(optionsReady) {
		...
		if(clear.isSelected())
		  output.setText("");
		if(!remoteIperf){
			iperf = new IperfThread(this,options,output);
			iperf.start();
		}
		else {
			if((sshServerField.getText().trim().length()==0)||(sshUsernameField.getText().trim().length()==0)||(sshPasswordField.getText().trim().length()==0)){
				JOptionPane.showMessageDialog(this, "Please enter the SSH server login information.","ERROR",JOptionPane.ERROR_MESSAGE,null);	
				stop.setEnabled(false);
			      run.setEnabled(true);
			}
			else {
				iperf = new IperfThread(this,options,output,sshServerField.getText().trim(),sshUsernameField.getText().trim(),sshPasswordField.getText().trim());
				iperf.start();
			}
		}
      }
	...
	try {
	  ...
	  JOptionPane.showMessageDialog(this, "Error Saving Output: " + fe.getMessage());
        }
      }
    }
  }
  ...
  boolean formOptions() {
	...
	i = new Integer(0);
      ...
      if(i.intValue() < 1) {
	JOptionPane.showMessageDialog(this, "For interesting Bandwidth graphs, please enter an interval to be 1 or greater.");
      return false;
      }
      ...
      if(v ==0 || v==1) {
	JOptionPane.showMessageDialog(this, "For Bandwidth graphs, adaptive format can not be selected");
	return false;
      }
    }
	...
	i = new Integer(0);
      ...
      if(i.intValue() < 1) {
	JOptionPane.showMessageDialog(this, "For interesting Jitter graphs, please enter an interval to be 1 or greater.");
	return false;
      }
      //can not have adaptive bits
      int v = formatList.getSelectedIndex();
	...
	//options += " -c " + dest.getText();
      ...
      if(dest.getText().trim().length() > 0) {
    	  options += " -c " + dest.getText();
      }
      else {
	JOptionPane.showMessageDialog(this, "Error: Please enter host to connect to.");
    ...
    if(transFile.getText().length() > 0 && transFile.isEnabled()) {
      ...
      if(!f.exists()) {
        JOptionPane.showMessageDialog(this, "Error: The file you selected does not exist.");
        return false;
      }
  ...
  public static void createBWGraph(Vector bw) {
    ...
    for(int i=0; i< bw.size(); ++i) {
      IperfStreamResult stream = (IperfStreamResult)bw.get(i);
  ...
  public void createJitterGraph(Vector bw) {
  ...
  public void changeState(boolean r, boolean s) {
    run.setEnabled(r);
    stop.setEnabled(s);
